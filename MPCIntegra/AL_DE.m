...
- (id)initDE:( ALAstOrbIntegFunction * ) afunc
{
    ...
    if (self) {
        ...
        unsigned int Neqn = [ IntegFunc AL_getNEQN ]; //afunc->GetNeqn();
                           
        yy    = ( double * )malloc(sizeof(double) * (Neqn + 1 ) ); //new double[ Neqn + 1 ];
        wt    = ( double * )malloc(sizeof(double) * (Neqn + 1 ) );//new double[ Neqn + 1 ];
        p     = ( double * )malloc(sizeof(double) * (Neqn + 1 ) );//new double[ Neqn + 1 ];
        yp    = ( double * )malloc(sizeof(double) * (Neqn + 1 ) );//new double[ Neqn + 1 ];
        ypout = ( double * )malloc(sizeof(double) * (Neqn + 1 ) );//new double[ Neqn + 1 ];
        
        phi = malloc( sizeof(double) * (Neqn + 1 ) );
        for(int i=0;i<(Neqn + 1 );i++)
            phi[i] = malloc( sizeof(double) * 17 );
        //phi   = ( double * *)malloc( ( sizeof(double) * (Neqn + 1 ) ) * (sizeof(double) * 17) );//new double*[ Neqn + 1 ];
...
- (void)dealloc
{
    ...
    free( yp );
    free( ypout );
    for(int i=0; i< [ IntegFunc AL_getNEQN ];i++)
        free( phi[i] );
    free( phi );
}

- (void) AL_DE_Intrp:(double) tx:(double *) y:(double) xout:(double *)yout:(double *)typout
{
    //
    // Variables
    //
    ...
    for (j=2;j<=ki;j++) {
        ...
        eta = hi/psijm1;
        limit1 = kip1 - j;
        for (i=1;i<=limit1;i++) tw[i] = gamma*w[i] - eta*tw[i+1];
        tg[j] = tw[1];
        rho[j] = gamma*rho[jm1];
        term = psijm1;
    }
    ...
    for (l=1;l<=[ IntegFunc AL_getNEQN ];l++) {
        typout[l] = 0.0;
        yout[l]  = 0.0;
    }
    ...
    for (j=1;j<=ki;j++){
        ...
        for (l=1;l<=[ IntegFunc AL_getNEQN ];l++) {
            ...
            typout[l] = typout[l] + temp3*phi[l][i];
        };
    };
    
    for (l=1; l<=[ IntegFunc AL_getNEQN ]; l++)  yout[l] = y[l] + hi*yout[l]; 
    //NSLog( @"Interp: yout %lf  %lf  %lf\n", yout[1] , yout[2], yout[3] );
}

//
// Integration step
//
-(void) AL_DE_Step:(double *) tx:(double *)y:(double *)eps:(bool *) crash
{
    //
    // Constants
    //
    ...
    // acceptable one.                                                   
    //                                                                   
    if (fabs(h) < fouru * fabs(*tx)) {
        h = sign(fouru*fabs(*tx),h);
        *crash = true;
        NSLog(@"Step error %i\n", 0);
        return;  // exit 
    };
    ...
    // acceptable value.                                  
    round = 0.0;
    for (l=1;l<= [ IntegFunc AL_getNEQN ] ;l++) round += (y[l]*y[l])/(wt[l]*wt[l]);
    ...
    if (start) {
        ...
        h = sign( max(absh, fouru*fabs(*tx)), h);
        hold = 0.0;
        hnew = 0.0;
        k = 1;
        kold = 0;
        ...
        if (p5eps<=100.0*round) {
            nornd = false;
            for (l=1;l<=[ IntegFunc AL_getNEQN ];l++) phi[l][15]=0.0;
        }
    }
    ...
    do {
        ...
        if (k>=nsp1) {
            for (i=nsp1;i<=k;i++) {
                temp1 = beta[i];
                for (l=1;l<=[ IntegFunc AL_getNEQN ];l++) phi[l][i] = temp1 * phi[l][i];
            }
        };
        ...
        if (nornd) {
            for (l=1;l<=[ IntegFunc AL_getNEQN ];l++) p[l] = y[l] + h*p[l];
        }
        else {
            for (l=1;l<=[ IntegFunc AL_getNEQN ];l++) {
                tau = h*p[l] - phi[l][15];
        ...
        if (!success) {
            ...
            *tx = xold;
            for (i=1;i<=k;i++) {
                temp1 = 1.0/beta[i];
                ip1 = i+1;
                for (l=1;l<=[ IntegFunc AL_getNEQN ];l++) phi[l][i]=temp1*(phi[l][i]-phi[l][ip1]);
            };
    ...
    temp1 = h*g[kp1];
    if (nornd) 
        for (l=1;l<=[ IntegFunc AL_getNEQN ];l++) y[l] = p[l] + temp1*(yp[l] - phi[l][1]);
    else 
        for (l=1;l<=[ IntegFunc AL_getNEQN ];l++) {
    ...
    if ( phase1 || (p5eps>=erk*two[k+1]) ) 
        ...
        if (p5eps<erk) {
            ...
            hnew = sign(max(hnew, fouru*fabs(*tx)), h);
        }
        else hnew = h;
    };
...
- (void) AL_DE_Integ:(double *) y:(double *) t:(double) tout:(double *) relerr:(double *) abserr:(DE_STATE *)State:(bool) PermitTOUT
{
    ...
    NSLog(@"AL_DE_Integ: %lf   eps: %lf\n",*t, eps );
  /*  
    if ( ( [ IntegFunc AL_getNEQN ]   <   1    ) ||             // invalid number of equations
    ...
    if  ( (*State==DE_INIT) || (!OldPermit) || (delsgn*del<=0.0) ) {
        ...
        start  = true;
        x      = *t;
        for (int l=1; l<=[ IntegFunc AL_getNEQN ]; l++) yy[l] = y[l];
        delsgn = sign(1.0, del);
    ...
    while (true) {  // Start step loop
        NSLog(@"Integration Step %i - %lf   %lf  %lf  %lf - absdel = %lf\n" , nostep, x, yy[1], yy[2], yy[3] , absdel );
        // If already past output point, interpolate solution and return
        if (fabs(x-*t) >= absdel) {
            NSLog(@"Normal Exit x=%lf  t=%lf  absdel=%lf\n", x, *t ,absdel);
            //Intrp (x, yy, tout, y, ypout);
            [ self AL_DE_Intrp:x :yy :tout :y :ypout ];
            NSLog( @"AInterp:  y %lf  %lf  %lf\n", y[1] , y[2], y[3] );
        ...
        if ( !PermitTOUT && ( fabs(tout-x) < fouru*fabs(x) ) ) {
            ...
            h = tout - x;
            [ IntegFunc AL_Run:x : yy :yp ];
            //GetIntegFunction()->Run(x,yy,yp);              // compute derivative yp(x)
            
            for (int l=1; l<=[ IntegFunc AL_getNEQN ]; l++) 
        ...
        //Step (x, yy, eps, crash);
        [ self AL_DE_Step:&x : yy : &eps : &crash ];
